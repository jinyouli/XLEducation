...
- (void)setState:(FooterPullRefreshState)aState{
	...
	switch (aState) {
		...
		case FooterPullRefreshNormal:
			...
			_statusLabel.text = NSLocalizedString(@"向下滑动刷新...", @"Pull down to refresh status");
...
- (void)FooterRefreshScrollViewDidScroll:(UIScrollView *)scrollView {
	...
	if (_state == FooterPullRefreshLoading) {
		...
		if (_state ==FooterPullRefreshPulling && scrollView.contentOffset.y > -65.0f && scrollView.contentOffset.y < 0.0f && !_loading) {
			[self setState:FooterPullRefreshNormal];
		} else if (_state == FooterPullRefreshNormal && scrollView.contentOffset.y < -65.0f && !_loading) {
