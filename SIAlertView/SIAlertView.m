...
+ (void)initialize
{
    ...
    appearance.titleOfFont = [UIFont boldSystemFontOfSize:20];
...
- (void)updateTitleLabel
{
	if (self.title) {
		if (!self.titleLabel) {
			...
			self.titleLabel.font = self.titleOfFont;
...
- (void)setupButtons
{
    ...
    for (NSUInteger j = 0; j < 2; j++) {
        ...
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 60+j*55, 300, 1)];
        view.backgroundColor = [UIColor colorWithRed:220.0f/255.0f green:220.0f/255.0f blue:220.0f/255.0f alpha:1.0];
...
- (void)setTitleFont:(UIFont *)titleOfFont
{
    if (_titleOfFont == titleOfFont) {
        return;
    }
    _titleOfFont = titleOfFont;
    self.titleLabel.font = titleOfFont;
    [self invaliadateLayout];
}
